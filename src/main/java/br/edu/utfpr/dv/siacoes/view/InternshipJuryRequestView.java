...
public class InternshipJuryRequestView extends ListView {

	public static final String NAME = "internshipjuryrequest";
	...
	protected void loadGrid() {
		...
		this.getGrid().getColumns().get(3).setWidth(150);
		this.getGrid().getColumns().get(4).setWidth(75);
		...
		try {
			...
			for(InternshipJuryRequest request : list) {
				...
				Object itemId = this.getGrid().addRow(request.getInternship().getStudent().getName(), request.getInternship().getCompany().getName(), request.getInternship().getSupervisor().getName(), request.getDate(), (request.isConfirmed() ? "Sim" : "Não"));
				this.addRowId(itemId, request.getIdInternshipJuryRequest());
			}
		} catch (Exception e) {
	...
	public void editClick(Object id) {
		try {
			...
			UI.getCurrent().addWindow(new EditInternshipJuryRequestWindow(request, false, this));
		} catch(Exception e){
			Logger.getGlobal().log(Level.SEVERE, e.getMessage(), e);
